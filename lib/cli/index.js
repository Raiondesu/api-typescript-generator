...
yargs(hideBin(process.argv))
    .command('generate <config>', 'generates API files according to the specified configuration file.', () => configurationFileArgument, async (argv) => {
    ...
    for (const generateConfig of config.generates) {
        switch (generateConfig.type) {
            case 'openapiClient':
                ...
                const files = (await openapiToTypescriptClient({
                    document,
                    generateConfig
                })).files;
                ...
                for (const directoryPath of allDirectories) {
                    try {
                        ...
                        throw new Error(`Could not create directory "${directoryPath}": ${e instanceof Error ? e.message : e}.`);
                    }
                }
    ...
    .command('check <config>', 'compares previously generated API files with schema according to the specified configuration file.', () => configurationFileArgument, async (argv) => {
    const config = await loadConfig(argv.config);
    for (const generateConfig of config.generates) {
        switch (generateConfig.type) {
            case 'openapiClient':
                if (!(await compareGenerationResult({
                    files: (await openapiToTypescriptClient({
