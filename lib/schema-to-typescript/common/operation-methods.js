import { arrayExpression, arrowFunctionExpression, assignmentPattern, blockStatement, callExpression, classProperty, expressionStatement, identifier, isValidIdentifier, memberExpression, objectExpression, objectPattern, objectProperty, returnStatement, stringLiteral, thisExpression, tsLiteralType, tsPropertySignature, tsStringKeyword, tsTypeAnnotation, tsTypeLiteral, tsTypeParameterInstantiation, tsTypeReference, tsUndefinedKeyword, tsUnionType } from '@babel/types';
...
import { extendDependenciesAndGetResult, extendDependencyImports, generateSchemaTypeAndImports } from '../../utils/dependencies';
...
function generateUniqueName(source, postfixes, usedNames) {
    ...
    let i = 0;
    const baseName = applyEntityNameCase(source, 'camelCase') || (isValidIdentifier(source) ? source : postfixes[0]) || 'param';
...
function isOperationParameterFromArgument(value) {
...
function generateDestructuringName(paramName, postfixes, usedInputNames) {
...
export function generateOperationMethods({ paths, serviceName, getModelData, commonHttpClientImportName, validationContext, operationsConfig: { generateOperationName, generateOperationJsDoc, generateOperationResultDescription, generateOperationParameterArgumentName, generateOperationRequestBodyArgumentName, mediaTypeArgumentName, generateOperationParameterJsDoc, generateOperationRequestBodyJsDoc, validateResponse, makeResponseValidationSchemasExtensible, responseBinaryType = 'blob' } = {}, operationImportPath, binaryTypes, jsDocRenderConfig }) {
    const dependencyImports = {};
    const getBinaryType = () => extendDependenciesAndGetResult(generateBinaryType(binaryTypes, operationImportPath), dependencyImports);
    ...
    function getParameterErrorLocation(param, path, httpMethod) {
        return (`${JSON.stringify(param.name)} at path ` +
            `${JSON.stringify(path)}, method ${httpMethod.toUpperCase()}: unsupported style :` +
            JSON.stringify(param.style));
    }
    function createIncompatibleParameterStyleError(param, path, httpMethod) {
    ...
    for (const [path, pathItem] of Object.entries(paths)) {
        for (const httpMethod of openApiHttpMethods) {
            ...
            const suggestedOperationMethodName = applyEntityNameCase(operation.operationId ?? `${httpMethod}:${path}`, 'camelCase');
            ...
            for (const parameter of parameters) {
                ...
                if (typeof schema !== 'boolean' && schema.const !== undefined) {
                    ...
                    const suggestedParameterName = generateUniqueName(parameter.name, [parameter.in, `${parameter.in} param`], usedInputNames);
                    const parameterName = generateUniqueName(generateOperationParameterArgumentName
                        ...
                        : suggestedParameterName, [], usedInputNames);
                    usedInputNames[parameterName] = true;
                    const paramJsDoc = extractJsDoc(parameter);
                    inputParameters[parameter.in].push({
                        paramName: parameter.name,
                        argumentName: parameterName,
                        destructuringName: generateDestructuringName(parameterName, [parameter.in, `${parameter.in} param`], usedInputNames),
                        optional: !parameter.required,
                        type: extendDependenciesAndGetResult(generateSchemaTypeAndImports({
                            schema,
                            sourceImportPath: operationImportPath,
                            getModelData,
                            getBinaryType
                        }), dependencyImports),
                        docs: renderJsDoc(generateOperationParameterJsDoc
            ...
            let requestBodyArgumentNames;
            let argumentExtensionType;
            ...
            if (requestBody) {
                ...
                const mediaTypesWithRequestBodyNames = mediaTypes.map(([mediaType, content]) => {
                    const requestBodySuggestedName = generateUniqueName(getUserFreiendlySchemaName(content.schema ?? true) ?? 'request body', [], usedInputNames);
                    return {
                        mediaType,
                        content,
                        requestBodyName: generateUniqueName(generateOperationRequestBodyArgumentName
                            ...
                            : requestBodySuggestedName, [], usedInputNames)
                    };
                });
                const allRequestBodyNames = R.uniq(mediaTypesWithRequestBodyNames.map(({ requestBodyName }) => requestBodyName)).sort(([a, b]) => a.localeCompare(b));
                ...
                if (mediaTypesWithRequestBodyNames.length > 1) {
                    for (const { mediaType, content, requestBodyName } of mediaTypesWithRequestBodyNames) {
                        ...
                        const mediaTypePropertySignature = tsPropertySignature(identifier(mediaTypeName), tsTypeAnnotation(mediaTypeArgumentType));
                        mediaTypePropertySignature.optional = isDefaultType;
                        const jsdoc = extractJsDoc({ ...requestBody, ...content });
                        additionUnion.types.push(tsTypeLiteral([
                            ...
                            ...allRequestBodyNames.map((requestBodyNameItem) => {
                                ...
                                const property = attachJsDocComment(tsPropertySignature(identifier(requestBodyName), tsTypeAnnotation(isCurrentRequestBody
                                    ? extendDependenciesAndGetResult(generateSchemaTypeAndImports({
                                        schema,
                                        sourceImportPath: operationImportPath,
                                        getModelData,
                                        getBinaryType
                                    }), dependencyImports)
                                    : tsUndefinedKeyword())), renderJsDoc(generateOperationRequestBodyJsDoc
                    ...
                    inputParameters['body'].push({
                        ...
                        docs: renderJsDoc(generateOperationRequestBodyJsDoc
                            ? generateOperationRequestBodyJsDoc({
                                suggestedJsDoc: jsdoc,
                                serviceName,
                                operation,
            ...
            if (inputParameters['path'].length > 0) {
                requestObject.properties.push(objectProperty(identifier('pathParams'), objectExpression(inputParameters['path'].map(renderParameter))));
            }
            if (inputParameters['query'].length > 0) {
                requestObject.properties.push(objectProperty(identifier('query'), objectExpression(inputParameters['query'].map(renderParameter))));
            }
            if (inputParameters['header'].length > 0) {
                requestObject.properties.push(objectProperty(identifier('headers'), objectExpression(inputParameters['header'].map(renderParameter))));
            }
            ...
            for (const { argumentName, destructuringName, type, optional, docs, defaultValue } of Object.values(inputParameters)
                .flatMap((params) => params)
                .filter(isOperationParameterFromArgument)) {
                argument.properties.push(objectProperty(identifier(argumentName), defaultValue
                    ? assignmentPattern(identifier(destructuringName), defaultValue)
                    : identifier(destructuringName), false, argumentName === destructuringName));
                if (type) {
                    const propertySignature = attachJsDocComment(tsPropertySignature(identifier(argumentName), tsTypeAnnotation(type)), docs);
                    propertySignature.optional = optional;
                    argumentType.members.push(propertySignature);
                }
            }
            argument.typeAnnotation = tsTypeAnnotation(argumentExtensionType ? mergeTypes(argumentType, argumentExtensionType) : argumentType);
            if (requestBodyArgumentNames && requestBodyArgumentNames.length > 0) {
                requestObject.properties.push(objectProperty(identifier('body'), requestBodyArgumentNames.length > 1
                    ? callExpression(memberExpression(identifier('commonHttpClient'), identifier('pickRequestBody')), requestBodyArgumentNames.map(identifier))
                    : identifier(requestBodyArgumentNames[0])));
            }
            const operationMethod = arrowFunctionExpression(argument.properties.length > 0
                ? [
                ...
                : [], blockStatement([
                returnStatement(operationReturn.wrapResultExpression(callExpression(memberExpression(callExpression(memberExpression(thisExpression(), identifier('getClientInstance')), []), identifier('request')), [requestObject])))
            ]));
            const operationMethodProperty = classProperty(identifier(operationName), operationMethod);
            operationMethod.returnType = tsTypeAnnotation(tsTypeReference(identifier('Promise'), tsTypeParameterInstantiation([operationReturn.type])));
