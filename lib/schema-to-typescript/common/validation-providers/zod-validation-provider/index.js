...
import { arrayExpression, arrowFunctionExpression, binaryExpression, blockStatement, callExpression, expressionStatement, identifier, ifStatement, isIdentifier, isImportSpecifier, isVariableDeclaration, logicalExpression, memberExpression, newExpression, numericLiteral, objectExpression, objectPattern, objectProperty, returnStatement, stringLiteral, templateElement, templateLiteral, tsTypeReference } from '@babel/types';
import * as R from 'ramda';
import { extendSchema } from '../../../../schemas/common';
import { cleanupSchema } from '../../../../utils/cleanup-schema';
import { addDependencyImport } from '../../../../utils/dependencies';
...
function zCall(name, args) {
    return callExpression(member(name), args);
}
function addIssue({ message, path, code = 'custom' }) {
    return expressionStatement(callExpression(memberExpression(identifier('ctx'), identifier('addIssue')), [
        objectExpression([
...
function blockStatementIfNecessary(statement) {
    return statement.type === 'BlockStatement' ? statement : blockStatement([statement]);
}
async function loadExportsFromFile(filename) {
        ...
        if (statement.type === 'ImportDeclaration') {
            for (const specifier of statement.specifiers) {
                if (isImportSpecifier(specifier)) {
                    if (specifier.imported.type !== specifier.local.type ||
...
export class ZodValidationProvider extends ValidationProvider {
    ...
    generateSchemaItem(originalSchema, context) {
        ...
        if ('oneOf' in schema && schema.oneOf) {
            ...
            return zCall('union', [
                arrayExpression(schema.oneOf.map((subSchema) => this.generateSchemaItem(extendSchema(flatSchema, subSchema), context)))
            ]);
        }
        if ('anyOf' in schema && schema.anyOf) {
            ...
            return zCall('union', [
                arrayExpression(schema.anyOf.map((subSchema) => this.generateSchemaItem(extendSchema(flatSchema, subSchema), context)))
            ]);
        }
        ...
        if (schema.type === 'array') {
            ...
            if (schema.prefixItems) {
                ...
                if (schema.minItems !== undefined) {
                    result = callExpression(memberExpression(result, identifier('superRefine')), [
                        arrowFunctionExpression([identifier('items'), identifier('ctx')], blockStatement([
                            ifStatement(binaryExpression('<', memberExpression(identifier('items'), identifier('length')), numericLiteral(schema.minItems)), blockStatement([
                                addIssue({
                                    message: stringLiteral(`Array must contain at least ${schema.minItems} element(s)`),
                                    path: [],
                                    code: 'too_small'
                                })
                            ]))
                ...
                if (schema.maxItems !== undefined) {
                    result = callExpression(memberExpression(result, identifier('superRefine')), [
                        arrowFunctionExpression([identifier('items'), identifier('ctx')], blockStatement([
                            ifStatement(binaryExpression('>', memberExpression(identifier('items'), identifier('length')), numericLiteral(schema.maxItems)), blockStatement([
                                addIssue({
                                    message: stringLiteral(`Array must contain at most ${schema.maxItems} element(s)`),
                                    path: [],
                                    code: 'too_big'
                                })
                            ]))
            ...
            if (schema.uniqueItems === true) {
                let processedItems = callExpression(memberExpression(identifier('items'), identifier('map')), [
                    arrowFunctionExpression([identifier('item')], callExpression(memberExpression(identifier('JSON'), identifier('stringify')), [
                        identifier('item')
                    ]))
                ]);
                if (schema.items &&
                ...
                result = callExpression(memberExpression(result, identifier('superRefine')), [
                    arrowFunctionExpression([identifier('items'), identifier('ctx')], blockStatement([
                        ifStatement(binaryExpression('!==', memberExpression(newExpression(identifier('Set'), [processedItems]), identifier('size')), memberExpression(identifier('items'), identifier('length'))), blockStatement([
                            addIssue({
        ...
        if (schema.type === 'object') {
            ...
            let result = zCall('object', [
                objectExpression(schema.properties
                    ? Object.entries(schema.properties).map(([key, value]) => {
                        let propertyValueType = this.generateSchemaItem(value, context);
                        if (!requiredIndex[key]) {
                            propertyValueType = callExpression(memberExpression(propertyValueType, identifier('optional')), []);
                        }
            ...
            if (schema.minProperties !== undefined) {
                result = callExpression(memberExpression(result, identifier('superRefine')), [
                    arrowFunctionExpression([identifier('obj'), identifier('ctx')], blockStatement([
                        ifStatement(binaryExpression('<', memberExpression(callExpression(memberExpression(identifier('Object'), identifier('keys')), [
                            identifier('obj')
                        ]), identifier('length')), numericLiteral(schema.minProperties)), blockStatement([
                            addIssue({
                                message: stringLiteral(`Should have at least ${schema.minProperties} properties`),
                                path: []
                            })
                        ]))
                    ]))
            ...
            if (schema.maxProperties !== undefined) {
                result = callExpression(memberExpression(result, identifier('superRefine')), [
                    arrowFunctionExpression([identifier('obj'), identifier('ctx')], blockStatement([
                        ifStatement(binaryExpression('>', memberExpression(callExpression(memberExpression(identifier('Object'), identifier('keys')), [
                            identifier('obj')
                        ]), identifier('length')), numericLiteral(schema.maxProperties)), blockStatement([
                            addIssue({
                                message: stringLiteral(`Should have at most ${schema.maxProperties} properties`),
                                path: []
                            })
                        ]))
                    ]))
    ...
    generateAssertCall(validationSchema, data) {
        return this.withDependencyImports(callExpression(memberExpression(validationSchema, identifier('parse')), [data]));
    }
    generateSetModelNameCall(validationSchema, modelName) {
        return this.withDependencyImports(callExpression(memberExpression(validationSchema, identifier('describe')), [stringLiteral(modelName)]));
    }
    generateOperationResponseSchema(responses) {
        function generateMediaTypeSchema(mediaType) {
            if (mediaType === '*/*') {
                return zCall('string', []);
            }
            else {
        ...
        if (responseEntries.length === 1) {
            ...
            if (mediaTypeEntries.length === 1) {
                ...
                return this.withDependencyImports(zCall('object', [
                    objectExpression([
                        objectProperty(identifier('status'), zCall('literal', [numericLiteral(parseInt(status, 10))])),
        ...
        const usedResponseProperties = {
            status: false,
            mediaType: false,
            body: false
        };
        function useResponseProperty(propertyName) {
            usedResponseProperties[propertyName] = true;
            return identifier(propertyName);
        }
        function generateValidateBodyCall(bodySchema) {
            return bodySchema
                ? expressionStatement(callExpression(memberExpression(bodySchema, identifier('parse')), [useResponseProperty('body')]))
                : returnStatement();
        }
        function generateIfStatementSequence(cases, buildCondition, buildStatement, otherwiseCallback) {
            ...
            cases = { ...cases };
            let otherwise = otherwiseCallback();
        ...
        function generateMediaTypeSwitch(mediaTypes) {
            let wildcardMediaTypeSchema = null;
            if (mediaTypes['*/*'] !== undefined) {
                wildcardMediaTypeSchema = mediaTypes['*/*'];
                mediaTypes = { ...mediaTypes };
                delete mediaTypes['*/*'];
            }
            return generateIfStatementSequence(mediaTypes, (mediaType) => binaryExpression('===', useResponseProperty('mediaType'), stringLiteral(mediaType)), (bodySchema, mediaType) => isJsonMediaType(mediaType)
                ...
                return blockStatement([
                    addIssue({
                        message: templateLiteral([templateElement({ raw: 'Unexpected media type: ' }), templateElement({ raw: '' })], [useResponseProperty('mediaType')]),
                        path: ['mediaType']
                    })
                ]);
            });
        }
        const suprtRefineBody = blockStatementIfNecessary(generateIfStatementSequence(responses, (status) => binaryExpression('===', useResponseProperty('status'), numericLiteral(parseInt(status, 10))), (mediaTypes) => blockStatementIfNecessary(generateMediaTypeSwitch(mediaTypes)), () => blockStatement([
            addIssue({
                message: templateLiteral([templateElement({ raw: 'Unexpected status code: ' }), templateElement({ raw: '' })], [useResponseProperty('status')]),
                path: ['status']
            })
        ])));
        return this.withDependencyImports(callExpression(memberExpression(zCall('object', [
            objectExpression([
            ...
            arrowFunctionExpression([
                objectPattern(Object.entries(usedResponseProperties)
                    .map(([key, value]) => value ? objectProperty(identifier(key), identifier(key)) : null)
                    .filter((prop) => prop !== null)),
                identifier('ctx')
            ], suprtRefineBody)
        ]));
    }
    async generateMakeExtensibleCallback() {
    ...
    async generateFormatErrorMessageCallback() {
