import { isBooleanLiteral, isIdentifier, isNumericLiteral, isStringLiteral, isTSCallSignatureDeclaration, isTSConstructSignatureDeclaration, isTSIntersectionType, isTSLiteralType, isTSMethodSignature, isTSPropertySignature, isTSTypeLiteral, isTSUnionType, tsBooleanKeyword, tsIntersectionType, tsPropertySignature, tsTypeAnnotation, tsTypeLiteral, tsUnionType } from '@babel/types';
import * as R from 'ramda';
function flattenUnions(union) {
...
function flattenIntersections(intersection) {
    const result = tsIntersectionType([]);
    let hasChanges = false;
...
function getPropertyKey(member) {
...
export function simplifyUnionTypeIfPossible(union) {
    const result = tsUnionType([]);
    ...
    while (itemsToProcess.length > 0) {
        ...
        for (let i = 0; i < itemsToProcess.length; i++) {
            ...
            if (R.equals(current, compared)) {
            ...
            else if (isTSLiteralType(current) &&
                ...
                if (isTSTypeLiteral(current) && isTSTypeLiteral(compared)) {
                    ...
                    const keysWithDifferences = Object.keys(currentProperties).filter((key) => !R.equals(currentProperties[key].typeAnnotation?.typeAnnotation, comparedProperties[key].typeAnnotation?.typeAnnotation));
                        ...
                        current.members = current.members.map((member) => {
                            if (isTSPropertySignature(member)) {
                                ...
                                if (key === keyWithDifference) {
                                    const propertySignature = tsPropertySignature(member.key, tsTypeAnnotation(simplifyUnionTypeIfPossible(flattenUnions(tsUnionType([
...
export function getUserFreiendlySchemaName(schema) {
...
function mergeTypeLiteralsIfPossible(first, second) {
    ...
    for (const [key, prop] of Object.entries(firstProperties)) {
        if (Object.prototype.hasOwnProperty.call(secondProperties, key)) {
            ...
            newProp.typeAnnotation = tsTypeAnnotation(mergeTypes(newProp.typeAnnotation.typeAnnotation, secondProperties[key].typeAnnotation.typeAnnotation));
...
export function simplifyIntersectionTypeIfPossible(intersection) {
    const result = tsIntersectionType([]);
...
export function isAssignableToEmptyObject(type) {
