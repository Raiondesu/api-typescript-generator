import { addComment, isTSPropertySignature, removeComments } from '@babel/types';
import { wordWrap } from './string-utils';
function processDescription(info) {
    return info
        .trim()
...
export function extractJsDoc(entity) {
    const result = { tags: [] };
...
export function renderJsDocList(items) {
...
export function renderJsDocAsPlainText(jsdoc) {
    const bits = [];
...
export function renderJsDoc(jsdoc, config = {}) {
...
export function attachJsDocComment(node, jsdoc) {
    if (jsdoc) {
        removeComments(node);
        if (!jsdoc.match(/\n/)) {
...
export function extractJsDocString(entity, params = []) {
